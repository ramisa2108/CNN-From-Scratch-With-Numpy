...
class Config(object):
    ...
    model_folder = '.'
    model_weight_file = os.path.join(model_folder, '1705004_model.pickle')
...
def load_datasets(image_folder, dataset_size=None):
    ...
    print("{} images loaded from {}".format(len(X), image_folder))
    return X
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2:
        raise Exception('Data folder path needed')
    dataset_folder = sys.argv[1]

    
    test_X = load_datasets(dataset_folder, conf.test_size)
    ...
    output_df.to_csv(os.path.join(dataset_folder, '1705004_prediction.csv'), index=False)